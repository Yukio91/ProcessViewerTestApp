...
namespace ProcessViewerTestApp.WinApi
{
    ...
    public static class ProcessUtilities
    {
        ...
        public static string GetCommandLineByHandle(IntPtr handle)
        {
            return GetProcessParametersStringByHandle(handle, Environment.Is64BitOperatingSystem ? 0x70 : 0x40);
        }
        ...
        {
            IntPtr handle = NativeMethods.OpenProcess(NativeMethods.ProcessAccessFlags.QueryInformation | NativeMethods.ProcessAccessFlags.VirtualMemoryRead/*PROCESS_QUERY_INFORMATION | PROCESS_VM_READ*/, processId);
            if (handle == IntPtr.Zero)
                throw new Win32Exception(Marshal.GetLastWin32Error());

            try
            {
                return GetProcessParametersStringByHandle(handle, offset);
            }
            finally
            {
        ...
        private static string GetProcessParametersStringByHandle(IntPtr handle, int offset)
        {
